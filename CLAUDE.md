...
- `docs/deployment.md` — CI/CD deployment: GitHub Actions pipeline deploying to Vercel with a Dockerized build, the two flows (PR → Vercel Preview, `main` → Production), required checks (lint, typecheck, tests, build), and how secrets, environment variables, and the MongoDB Atlas production connection are managed. Read before touching CI/CD, deployment config, or environment/secret setup.
